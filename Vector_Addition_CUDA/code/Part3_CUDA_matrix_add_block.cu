 ...
 * Step3: input nvcc -o addition2 Part3_CUDA_matrix_add_block.cu
 * Step4: input ./addition2
 *
*/

/* 2. In this program, it will conduct experiment 4: 
  1) Experiment 4: configurations. Varying #block, #Threads 1024, descend blocks 
    std::vector<size_t> numBlocksList4 = {1024, 512,256,128, 32, 8, 1};
    std::vector<size_t> size4 = {4, 6,7, 9, 10, 11,12, 15,18,20};
    std::ofstream outputFile41("experiment_block_op_41.txt");
    std::ofstream outputFile42("experiment_block_m_42.txt");
 * 
*/
...
int main() {
    ...
    std::ofstream outputFile21("experiment_block_op.txt");
    std::ofstream outputFile22("experiment_block_m.txt");


  	
  	// Experiment 2: Varying #Blocks, #Threads 1024
  	// size2 = size4 = {4, 6,7, 9, 10, 11,12, 15,18,20}
  	for (int i = 0; i < size4.size(); i++) {
  		
      // Calculate vectorsize
  		size_t exponent = size4[i];
          ...
          if (vectorSize == 16) {
            ...
            printVector(vectorA, "Vector A Before experiments 4, size == 16: ");
            
            // Print vectorB before kernel execution
            printVector(vectorB, "Vector B Before experiments 4, size == 16: ");
            
            // Print resultVector before kernel execution
            printVector(resultVector, "resultVector Before experiments 4, size == 16: ");
            std::cout << std::endl;
          }
          
          // numBlocksList4 = {1024, 512,256,128, 32, 8, 1} 
          ...
          if (vectorSize == 16) {
              ...
              printVector(vectorA, "Vector A experiments 4, size == 16: :");
              printVector(vectorB, "Vector B experiments 4, size == 16: :");
              printVector(resultVector, "resultVector experiments 4, size == 16:");
  			      std::cout << std::endl;
          }

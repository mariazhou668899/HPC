 ...
 * Step3: Add mmM_S_Tran_Unroll.cpp to the Resource Files of this project.
 * Step4: run program
 ...
 * Step2: open cmd, navigate to the file holding mmM_S_Tran_Unroll.cpp
 * Step3: input g++ mmM_S_Tran_Unroll.cpp -o s_Tran_Unroll.exe -L/path/to/include -L/path/to/lib -lopenblas
 * -----  For example: g++ mmM_S_Tran_Unroll.cpp -o s_Tran_Unroll.exe -I"D:/xzAssignment/software/OpenBLAS-0.3.26-x64/include" -L"D:/software/OpenBLAS-0.3.26-x64/lib" -lopenblas
 * Step4: input s_Tran_Unroll.exe
 * Step5: check the results from screen or from the file which is generated in the same folder of g++ mmM_S_Tran_Unroll.cpp
 *
*/

/*  * Introduction: mmM_S_Tran_Unroll.cpp contains the solution implementation.
 ...
 *     in the same folder of mmM_S_Tran_Unroll.cpp by g++.
...
int main() {
    ...
    for (int dimension = 10; dimension <= 1000; dimension += 1) {
        ...
        if (rows <= 15) {
            ...
            //Output the result calculated by matrix-vector-multiply transpose and unrolled sequential solution 
